...
char *add_device(char *device_name, char *bpf_expr, int num, int promisc, int to_ms, char *options);
...
static bool process_packet(struct device_map *device, gint64 offset,
                           const struct pcap_pkthdr *pkthdr, const u_char *packet, int printCJson);
...
void process_sockaddr(const struct sockaddr *sockaddr, char *buffer, size_t buffer_size) {
    if (sockaddr == NULL) {
        buffer[0] = '\0';
        return;
    }
    ...
    switch (sockaddr->sa_family) {
        ...
        case AF_PACKET: {  // Linux
            ...
            snprintf(buffer, buffer_size, "%02x:%02x:%02x:%02x:%02x:%02x", sll->sll_addr[0],
        ...
        case AF_LINK: {  // macOS
            ...
            if (sdl->sdl_alen == 6) {
                ...
                snprintf(buffer, buffer_size, "%02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1],
        ...
        default:  // Unsupported address type
            snprintf(buffer, buffer_size, "Unsupported Address Type: %d", sockaddr->sa_family);
            break;
    }
}
...
char *get_if_list() {
    ...
    for (pcap_if_t *pdev = alldevs; pdev != NULL; pdev = pdev->next) {
        ...
        int written =
            snprintf(device_buffer, sizeof(device_buffer),
...
char *init_cf_live(capture_file *cf_live, char *options) {
    if (!is_empty_json(options)) {
        ...
        const cJSON *keysListJson = cJSON_GetObjectItemCaseSensitive(json, "tls.keys_list");
        ...
        const cJSON *desegmentSslApplicationDataJson =
            cJSON_GetObjectItemCaseSensitive(json, "tls.desegment_ssl_application_data");
...
static bool send_data_to_wrap(struct device_map *device, int printCJson) {
    ...
    if (json_str && dataCallback != NULL) {
        ...
        LOG_DEBUG("Skipping Go callback (json_str: %p, cb: %p)", json_str, dataCallback);
    }
...
static bool process_packet(struct device_map *device, gint64 offset,
    ...
    frame_data_init(&fd, device->content.cf_live->count, &device->content.rec, offset, cum_bytes);
    ...
    // dissect pkg
    epan_dissect_run_with_taps(&device->content.edt, device->content.cf_live->cd_t,
                               &device->content.rec, &fd, &device->content.cf_live->cinfo);
 ...
 */
char *handle_packet(char *device_name, char *bpf_expr, int num, int promisc, int to_ms,
                    int printCJson, char *options) {
    ...
    device->content.handle = pcap_open_live(device->device_name, SNAP_LEN, device->content.promisc,
    ...
    if (pcap_lookupnet((const char *)device->device_name, &net, &mask, err_buf) != 0) {
        fprintf(stderr, "Could not get netmask for device %s: %s\n", device->device_name, err_buf);
        net = 0;
        mask = 0;
    }

    if (pcap_compile(device->content.handle, &fp, device->content.bpf_expr, 0, net) != 0) {
    ...
    int loop_ret = pcap_loop(device->content.handle, device->content.num, process_packet_callback,
                             (u_char *)&args);

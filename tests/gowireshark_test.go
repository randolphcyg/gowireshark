...
func TestDissectPktLiveSpecificNum(t *testing.T) {
	...
	go func() {
		for {
			select {
			case pkg := <-gowireshark.DissectResChans[ifName]:
				livePkgCount++

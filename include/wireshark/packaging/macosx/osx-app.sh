...
qt_frameworks_dir=$( "/usr/lib/aarch64-linux-gnu/qt5/bin/qmake" -query QT_INSTALL_LIBS )
...
pkgplugin="$bundle/Contents/PlugIns/$app_lower/4.0"
...
codesign_file () {
	...
	codesign \
		...
		--force \
		--options runtime \
		--entitlements "/opt/wireshark/packaging/macosx/entitlements.plist" \
		--timestamp \
		--verbose \
		"$1" || exit 1
}
